...
public class WalletActivity extends BaseActivity implements WalletIView {
        ...
        if (requestCode == PaymentActivity.PICK_PAYMENT_METHOD && resultCode == Activity.RESULT_OK && data != null)
            switch (data.getStringExtra("payment_mode")) {
                case "CARD":
                    ...
                    Log.d("Payment", "onActivityResult: "+map.toString());
                    showLoading();
                ...
                case Constants.PaymentMode.PAYTM: {
                    ...
                    hashMap.put("user_type", "Exzytaxi");
                    showLoading();
    ...
    public void onSuccess(User user) {
      try {
          SharedHelper.putKey(this, "walletBalance", user.getWalletBalance() + "");
          walletBalance.setText(getNumberFormat().format(Double.parseDouble(SharedHelper.getKey(this, "walletBalance", "0"))));
      }catch (Exception e){
            e.printStackTrace();
      }
